...
async function deployDirectDemocracyToken(wallet) {
  const DirectDemocracyTokenFactoryBytecode =
    DirectDemocracyTokenFactory.bytecode;
  const DirectDemocracyTokenFactoryAbi = DirectDemocracyTokenFactory.abi;

  const factory = new ethers.ContractFactory(
    DirectDemocracyTokenFactoryAbi,
...
async function deployDirectDemocracyVoting(wallet, ddtokenAddress) {
  const DirectDemocracyVotingFactoryBytecode =
    DirectDemocracyVotingFactory.bytecode;
  const DirectDemocracyVotingFactoryAbi = DirectDemocracyVotingFactory.abi;

  const factory = new ethers.ContractFactory(
    DirectDemocracyVotingFactoryAbi,
...
async function makeNFTMembership(
  ...
  const nftMembershipContract = new ethers.Contract(
    lastDeployedContractAddress,
    NFTMembership.abi,
    nftFactoryContract.signer
  );
...
async function makeDDToken(
  ddTokenFactoryContract,
  name,
  symbol,
  ...
  const tx = await ddTokenFactoryContract.createDirectDemocracyToken(
    name,
    symbol,
    nftMembershipAddress,
    allowedRoleNames,
  ...
  const ddTokenContract = new ethers.Contract(
    ddTokenAddress,
    DirectDemocracyToken.abi,
    ddTokenFactoryContract.signer
  );
...
async function makePTToken(ptTokenFactoryContract, name, symbol, POname) {
  ...
  const tx = await ptTokenFactoryContract.createParticipationToken(
    name,
    symbol,
    POname
  );
...
async function makeTreasury(treasuryFactoryContract, POname) {
  ...
  try {
    ...
    const treasuryCreatedEvent = txReceipt.events?.find(
      (event) => event.event === "TreasuryCreated"
    );
...
async function makeParticipationVoting(
  ...
  if (
    !participationVotingFactoryContract ||
    !ethers.utils.isAddress(participationTokenAddress) ||
    !ethers.utils.isAddress(nftMembershipAddress) ||
    !ethers.utils.isAddress(treasuryAddress)
  ) {
    console.error(
      "Invalid parameters provided to createParticipationVoting function"
    );
    return;
  }
  ...
  try {
    const txResponse =
      await participationVotingFactoryContract.createParticipationVoting(
        participationTokenAddress,
        nftMembershipAddress,
        allowedRoleNames,
        quadraticVotingEnabled,
    ...
    const votingContractCreatedEvent = txReceipt.events?.find(
      (event) => event.event === "VotingContractCreated"
    );
    const votingContractAddress =
      votingContractCreatedEvent.args.votingContractAddress;

    console.log(
      `Participation Voting Contract deployed at address: ${votingContractAddress}`
    );
...
async function makeHybridVoting(
  ...
  if (
    !hybridVotingFactoryContract ||
    !ethers.utils.isAddress(participationTokenAddress) ||
    !ethers.utils.isAddress(democracyTokenAddress)
  ) {
  ...
  try {
    ...
    const hybridVotingCreatedEvent = txReceipt.events?.find(
      (event) => event.event === "HybridVotingContractCreated"
    );
    const newHybridVotingAddress =
      hybridVotingCreatedEvent.args.hybridVotingAddress;

    console.log(
      `Hybrid Voting contract created at address: ${newHybridVotingAddress}`
    );
...
async function makeTaskManager(
  ...
  const tx = await taskManagerFactoryContract.createTaskManager(
    tokenAddress,
    nftMembershipAddress,
    allowedRoleNames,
    POname
  ...
  const receipt = await tx.wait();
  const taskManagerCreatedEvent = receipt.events?.filter(
    (x) => x.event === "TaskManagerCreated"
  )[0];
  if (taskManagerCreatedEvent) {
    ...
    console.error(
      "Task Manager address not found from the transaction receipt"
    );
  }
}

async function makeDirectDemocracyVoting(
  ddVotingFactoryContract,
  ddTokenAddress,
  nftMembershipAddress,
  allowedRoleNames,
  treasuryAddress,
  ...
  if (
    ...
    console.error(
      "Invalid parameters provided to createDirectDemocracyVoting function"
    );
    return;
  }

  const tx = await ddVotingFactoryContract.createDirectDemocracyVoting(
    ddTokenAddress,
    nftMembershipAddress,
    allowedRoleNames,
    treasuryAddress,
  ...
  const votingCreatedEvent = receipt.events?.filter(
    (event) => event.event === "VotingContractCreated"
  )[0];
  if (votingCreatedEvent) {
    ...
    console.error(
      "Direct Democracy Voting address not found from the transaction receipt"
    );
  }
}
...
const makeRegistry = async (
  ...
  try {
    const txResponse = await registryFactoryContract.createRegistry(
      votingControlAddress,
      contractNames,
      contractAddresses,
      POname,
    ...
    const registryCreatedEvent = txReceipt.events?.find(
      (event) => event.event === "RegistryCreated"
    );
